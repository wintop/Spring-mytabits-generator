 ...
 * 生成类
 */
public class ClassAdapter extends ClassDefine {
    private List<Annotation> annotations;
    private ClassBody body;

    public ClassAdapter(ClassType classType, String name) {
        super(Modifier.PUBLIC, classType, name);
    }

    public ClassAdapter(Modifier modifier, ClassType type, String name) {
        super(modifier, type, name);
    }

    public ClassAdapter(Modifier modifier, ClassType type, String name, String template,
    ...
    private int removeSameImport() {
        ...
        if (!GenUtils.isEmpty(this.getImports().get())) {
    ...
    public String formatString() {
        ...
        sb.append(this.getPkgName().getPackage()).append(";")
                .append(GenUtils.line(2))
                .append(this.getImports().formatString());

        if (!GenUtils.isEmpty(this.comment)) {
            sb.append("/**")
                    .append(GenUtils.line())
        ...
        if (!GenUtils.isEmpty(this.getAnnotations())) {
            sb.append(GenUtils.formatList(this.getAnnotations()));
        }
