<template>
  <div class="main-wrapper">
    ...
    <el-dialog title="详情" width="60%" :visible.sync="dialogCopyVisible">
      <template>
        <el-transfer
          v-model="columnSelection"
          :props="{ key: 'columnName', label: 'remarks' }"
          :data="columnList"
        />
      </template>
      <div slot="footer" class="dialog-footer">
        <el-button type="primary" @click="copyToTableColunm">复制数据库记录</el-button>
        <el-button @click="dialogCopyVisible = false">关闭</el-button>
      </div>
    </el-dialog>
  </div>
</template>
...
<script>
import { getTableColumnList, addTableColumn, editTableColumn, getColumnFromDb, copyColumnToTable } from '@/api/table-column'
...
export default {
  ...
  created() {
    this.tableId = this.$route.params && this.$route.params.id
    this.fetchTableColumnList(this.tableId)
  },
  methods: {
    fetchTableColumnList(tableId) {
      this.listLoading = true
      getTableColumnList(tableId).then(response => {
    ...
    handleDbColumn() {
      getColumnFromDb(this.tableId).then(res => {
        this.columnList = res.data
        if (this.list) {
          this.list.forEach(item => {
            this.columnSelection.push(item.name)
          })
        }
    ...
    copyToTableColunm() {
      if (this.columnSelection && this.columnSelection.length > 0) {
        const selectList = []
        this.columnSelection.forEach(item => {
          this.columnList.forEach(element => {
            if (item === element.columnName) {
              selectList.push(element)
            }
          })
        })
        console.log(selectList)
        copyColumnToTable(selectList).then(res => {
          console.log(res)
        })
      }
    }
